...
//#endregion
//#region .temp/constants/index.js
var require_constants = /* @__PURE__ */ __commonJS({ ".temp/constants/index.js": ((exports) => {
	var __importDefault$1 = exports && exports.__importDefault || function(mod) {
		return mod && mod.__esModule ? mod : { "default": mod };
	};
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.defaultDecimalConfigs = void 0;
	const decimal_js_1$1 = __importDefault$1(require_decimal());
	exports.defaultDecimalConfigs = {
		precision: 16,
		rounding: decimal_js_1$1.default.ROUND_HALF_UP,
		toExpNeg: -7,
		toExpPos: 21,
		maxE: 9e15,
		minE: -9e15,
...
//#endregion
//#region .temp/utils/index.js
var require_utils = /* @__PURE__ */ __commonJS({ ".temp/utils/index.js": ((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });
	exports.getDecimalPlaces = exports.defaultDecimalConfig = exports.isObject = exports.isString = exports.isNumber = void 0;
	...
	const getDecimalPlaces = (originNumber, options) => {
		const defaultDecimal = options?.defaultDecimal || 0;
		const maxDecimal = (0, exports.isNumber)(options?.maxDecimal) ? options.maxDecimal : constants_1$1.defaultDecimalConfigs.precision;
		if (typeof originNumber !== "number" && typeof originNumber !== "string" || Number.isNaN(originNumber)) {
			console.warn("请传入正确的数据类型，应为 Number 或 String 类型的数字");
			return defaultDecimal;
		}
		const numString = (0, exports.isNumber)(originNumber) ? Number(originNumber).toString() : originNumber;
		if (numString.includes("e-")) {
			...
			let decimalPlaces$1;
			if (frontDigits === "1" && exponent > 0) decimalPlaces$1 = exponent;
			else decimalPlaces$1 = exponent + frontDigits.length - (frontPart.includes(".") ? 1 : 0);
			return Math.min(decimalPlaces$1, maxDecimal);
		}
...
var require_main = /* @__PURE__ */ __commonJS({ ".temp/main.js": ((exports) => {
	...
	Object.seal(constants_1.defaultDecimalConfigs);
	const cacheFnList = [
		"all",
		"sum",
		"subtractMultiple",
	...
	var Calculator = class Calculator {
		constructor() {
			...
			this.calcConfigs = { ...constants_1.defaultDecimalConfigs };
		}
		resetInstance() {
			this.clearCache();
			this.userOptions = { ...defaultUserOptions };
		...
		getThisDataMaxPrecision(dataStructure) {
			...
			if (Array.isArray(dataStructure)) return Math.max(...dataStructure.map((v) => (0, utils_1.getDecimalPlaces)(v)));
			else if (typeof dataStructure === "object" && dataStructure !== null) return Math.max(...Object.values(dataStructure).map((v) => (0, utils_1.getDecimalPlaces)(v)));
			else if (typeof dataStructure === "number") if (dataStructure % 1 !== 0) return (0, utils_1.getDecimalPlaces)(dataStructure);
		...
		generateCacheKey(data) {
			const stableStringify = (obj) => {
				...
				if (!(0, utils_1.isObject)(obj)) return JSON.stringify(obj);
		...
		sum(data, userOptions) {
			...
			if (mergedOptions !== null && (0, utils_1.isObject)(userOptions) && Object.keys(userOptions).length > 0) Object.entries(userOptions).forEach(([key, val]) => {
				if (key in this.userOptions) mergedOptions[key] = val;
			});
			const DecimalClone = decimal_js_1.default.clone({ ...constants_1.defaultDecimalConfigs });
			...
			if (Array.isArray(data)) {
				if (data.includes(Infinity) || data.includes(-Infinity)) console.error("Infinity or -Infinity is not allowed in the array.");
				numbersToSum = data.filter((num) => (0, utils_1.isNumber)(num) && !Number.isNaN(num) && num !== Infinity && num !== -Infinity);
			} else if ((0, utils_1.isObject)(data)) numbersToSum = Object.values(data).filter((value) => (0, utils_1.isNumber)(value) && !Number.isNaN(value) && value !== Infinity && value !== -Infinity);
			else if ((0, utils_1.isNumber)(data)) numbersToSum = [data];
		...
		subtractMultiple(initialValue, subtractValues, userOptions) {
			...
			if (!(0, utils_1.isNumber)(initialValue) || Number.isNaN(initialValue)) initialValue = 0;
			if (!Array.isArray(subtractValues)) subtractValues = [subtractValues];
			const filteredSubtractValues = subtractValues.filter((v) => (0, utils_1.isNumber)(v));
		...
		calcLinePrice(calcBaseTotalParams, userOptions) {
			...
			if (!(0, utils_1.isNumber)(quantity) || Number.isNaN(quantity)) quantity = null;
			if (!(0, utils_1.isNumber)(unitPrice) || Number.isNaN(unitPrice)) unitPrice = null;
			if ((0, utils_1.isNumber)(quantity) && quantity <= 0) {
				console.warn("参数错误, quantity 必须大于等于 0。这里按 0 处理来处理");
				quantity = 0;
			}
			if (quantity === null && (0, utils_1.isNumber)(unitPrice) && unitPrice >= 0) return {
				quantity,
				unitPrice,
				linePrice: unitPrice
			};
			if ((0, utils_1.isNumber)(quantity) && quantity >= 0 && unitPrice === null) return {
				quantity,
				unitPrice: null,
				linePrice: null
			};
		...
		calculateDiscountedPrice(originalPrice, discountRate, userOptions) {
			if (originalPrice === null || discountRate === null || !(0, utils_1.isNumber)(originalPrice) || Number.isNaN(originalPrice) || !(0, utils_1.isNumber)(discountRate) || Number.isNaN(discountRate)) return null;
			if ((0, utils_1.isNumber)(originalPrice) && originalPrice < 0) {
				console.error("参数 originalPrice 应该为大于 0 的数值");
				return originalPrice;
			}
			if (discountRate === 0) return originalPrice;
			if (discountRate === 1) return 0;
			if ((0, utils_1.isNumber)(discountRate) && discountRate < 0) {
		...
		percentToDecimal(originPercentage, userOptions) {
			...
			else if (!(0, utils_1.isObject)(userOptions) && !(0, utils_1.isNumber)(userOptions)) userOptions = curUserOptions;
			else if ((0, utils_1.isNumber)(userOptions)) {
				const tempNum = Number.isNaN(userOptions) ? userOptions : -1;
				userOptions = { outputDecimalPlaces: tempNum };
			}
			if ((0, utils_1.isObject)(userOptions)) Object.entries(userOptions).forEach(([key, val]) => {
				curUserOptions[key] = val;
			});
			if (originPercentage === null || !(0, utils_1.isNumber)(originPercentage) || Number.isNaN(originPercentage)) return null;
			const DecimalClone = decimal_js_1.default.clone({ ...constants_1.defaultDecimalConfigs });
			const cacheKey = this.generateCacheKey({
				originPercentage,
				curUserOptions
		...
		decimalToPercent(originNumber, userOptions) {
			const curUserOptions = { ...this._getUserOptions() };
			if (userOptions === void 0) userOptions = curUserOptions;
			else if (!(0, utils_1.isObject)(userOptions) && !(0, utils_1.isNumber)(userOptions)) {
			...
			if (originNumber === null || originNumber === 0 || !(0, utils_1.isNumber)(originNumber) || Number.isNaN(originNumber)) return 0;
			if (!(0, utils_1.isNumber)(userOptions?.outputDecimalPlaces) || Number.isNaN(userOptions?.outputDecimalPlaces)) {
		...
		computeRate(originPrice, param2, param3) {
			...
			if (originPrice === null || originPrice === 0 || !(0, utils_1.isNumber)(originPrice) || Number.isNaN(originPrice)) return 0;
			if (args.length === 1 && (0, utils_1.isNumber)(args[0])) {
				userRate = curUserOptions.taxRate;
				curUserOptions.rateType;
				userOptions = { ...curUserOptions };
			} else if (args.length === 2 && (0, utils_1.isNumber)(param2)) {
				userRate = param2;
				curUserOptions.rateType;
				userOptions = {
					...curUserOptions,
			...
			} else if (args.length === 2 && (0, utils_1.isObject)(param2)) {
			...
			} else if (args.length === 2 && (!(0, utils_1.isNumber)(param2) || !(0, utils_1.isObject)(param2))) {
				console.error("param2 传参异常。请参考API文档传入正确的配置项");
				return originPrice;
			} else if (args.length === 3 && (0, utils_1.isNumber)(param2) && (0, utils_1.isString)(param3)) {
				userRate = param2;
				userOptions = {
					...curUserOptions,
					taxRate: param2,
...
var require___temp = /* @__PURE__ */ __commonJS({ ".temp/index.js": ((exports) => {
	Object.defineProperty(exports, "__esModule", { value: true });

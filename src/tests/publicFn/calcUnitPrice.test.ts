...
describe('calcUnitPrice()', () => {
  ...
  describe('自定义精度测试', () => {
    ...
    it('验证禁用小数后的运算结果 -> 10 / 3=3.333 → 保留0位小数后为3', () => {
      ...
      expect(
        CalcInst.calcUnitPrice({ quantity: 3, linePrice: 10 } as CalcBaseTotalParams)
      ).toEqual({
        quantity: 3,
        unitPrice: 3,
        linePrice: 10,
    ...
    it('高精度场景下验证，设置6位 -> 10 / 6 = 1.666667', () => {
      ...
      expect(
        CalcInst.calcUnitPrice({ quantity: 6, linePrice: 10 } as CalcBaseTotalParams)
      ).toEqual({
        quantity: 6,
        unitPrice: 1.666667,
        linePrice: 10,
    ...
    it('验证更高位小数的四舍五入处理', () => {
      expect(
        CalcInst.calcUnitPrice(
          { quantity: 3, linePrice: 10.0005 } as CalcBaseTotalParams,
